...
## 22. SDK 模式与 Proxy 模式详解

LiteLLM 提供两种使用模式，本质是同一个引擎的不同打包方式。理解两者的区别和参数体系，是正确选型的前提。

### 22.1 两种模式概述

| 维度 | SDK 模式 | Proxy 模式 |
|------|---------|-----------|
| 形态 | `pip install litellm`，在应用中直接 import | 独立部署的 HTTP 服务，对外暴露 OpenAI 兼容 API |
| 进程 | 嵌入业务进程内 | 独立进程/容器 |
| 调用方式 | `litellm.acompletion(model=..., messages=...)` | `curl /v1/chat/completions`（OpenAI SDK 兼容） |
| 配置方式 | Python 代码中传参 | `config.yaml` + 环境变量 + DB |
| 部署依赖 | 无 | 可选 PostgreSQL + Redis |
| 多实例共享状态 | 需自建（或 Redis） | 内置 Redis 支持 |
| 治理能力（Key/预算） | 无，需自建 | 内置（虚拟 Key、spend tracking、UI 控制台） |
| 适合场景 | 库集成、微服务内部调用、自建网关 | 企业统一 AI 入口、多团队治理 |

**核心关系**：Proxy 模式在底层就是 `litellm.Router` + FastAPI 的一层包装。Proxy 的 `config.yaml` 最终被解析为 `Router(model_list=...)` 的相同参数结构。

```mermaid
graph LR
    subgraph Proxy["Proxy 模式"]
        YAML["config.yaml"] --> Parser[配置解析器]
        Parser --> Router[litellm.Router]
        Router --> FastAPI["FastAPI HTTP Server"]
    end

    subgraph SDK["SDK 模式"]
        Code["业务代码"] --> Router2[litellm.Router]
    end

    Router -->|相同的核心里程| Engine["LiteLLM 引擎\n协议适配 / 调用 / 重试"]
    Router2 --> Engine
```

### 22.2 SDK 模式核心参数

SDK 模式的核心入口有两个：`litellm.acompletion()` 直接调用 和 `litellm.Router` 带负载均衡的调用。

#### 22.2.1 `litellm.acompletion()` / `completion()` 参数

直接调用模式，一次调用对应一个指定的供应商模型。

```python
response = await litellm.acompletion(
    # === 必填 ===
    model="openai/gpt-4o",          # str: provider/model 格式
    messages=[{"role": "user", "content": "Hello"}],

    # === 认证（可在环境变量中预设） ===
    api_key="sk-...",               # Optional[str]: 供应商 API Key
    api_base="https://...",         # Optional[str]: 自定义 endpoint

    # === 推理参数 ===
    temperature=0.7,                # Optional[float]: 采样温度
    max_tokens=4096,                # Optional[int]: 最大输出 token
    top_p=0.9,                      # Optional[float]: 核采样
    stop=["\n"],                    # Optional[list]: 停止词
    stream=False,                   # Optional[bool]: 是否流式
    stream_options={"include_usage": True},  # Optional[dict]: 流式选项

    # === 超时 & 重试 ===
    timeout=60,                     # Optional[float/int]: 超时秒数
    num_retries=0,                  # Optional[int]: 自动重试次数

    # === 供应商特定参数 ===
    organization="org-...",         # OpenAI 组织 ID
    user="user-123",                # 终端用户标识
    extra_headers={"X-Custom": "v"},  # 自定义请求头
    extra_body={"anthropic_version": "vertex-2023-10-16"},  # 透传 body 字段
    mock_testing_rate_limit_error=False,  # 测试用：模拟限流错误

    # === 成本 & 用量 ===
    metadata={"trace_id": "abc"},   # Optional[dict]: 附加元数据
    input_cost_per_token=0.0,       # Optional[float]: 自定义输入单价
    output_cost_per_token=0.0,      # Optional[float]: 自定义输出单价
)
```

#### 22.2.2 `Router` 构造函数参数

Router 是进程内实例，管理模型列表和部署状态。
...
router = Router(
    # === 必填 ===
    model_list=[                    # list[dict]: 模型部署列表
        {
            "model_name": "gpt-4o",     # 对外模型名/模型组名
            "litellm_params": {
                "model": "openai/gpt-4o",
                "api_key": "sk-...",
                "api_base": "https://api.openai.com/v1",
                "rpm": 10000,           # 每分钟请求上限（用于加权负载均衡）
                "tpm": 500000,          # 每分钟 token 上限
                "weight": 1,            # 加权选择权重（仅 simple-shuffle 生效）
                "max_parallel_requests": 10,  # 最大并发数
                "timeout": 30,
                "order": 1,             # 优先级：小值优先
            },
            "model_info": {
                "id": "deploy-001",     # 部署唯一 ID
                "mode": "chat",
                "base_model": "gpt-4o",
                "max_input_tokens": 128000,
    ...
    routing_strategy="simple-shuffle",
    # 可选值:
    #   "simple-shuffle"           - 加权随机（默认，推荐生产）
    #   "least-busy"               - 最少并发
    #   "usage-based-routing"      - 用量感知（需 Redis）
    #   "usage-based-routing-v2"   - 用量感知 v2（异步，需 Redis）
    #   "latency-based-routing"    - 延迟优先
    #   "cost-based-routing"       - 成本优先
    routing_strategy_args={"ttl": 60},   # 策略特定参数（例如延迟窗口）

    # === 预检查 ===
    enable_pre_call_checking=True,  # bool: 调用前检查上下文窗口/区域
    enable_pre_call_checks=True,    # 同上，旧版命名

    # === 重试 ===
    num_retries=0,                  # int: 失败重试次数（全局）
    retry_after=0,                  # float: 重试前最小等待秒数
    retry_policy=RetryPolicy(       # 按异常类型的重试策略
    ...
    # === 熔断 ===
    allowed_fails=3,                # int: 每分钟允许失败次数，超出后冷却
    cooldown_time=5,                # int: 冷却时长（秒）
    disable_cooldowns=False,        # bool: 关闭冷却
    ...
    # === 超时 ===
    timeout=30,                     # float: 全局超时（秒）
    request_timeout=30,             # 同上

    # === 缓存 ===
    cache_responses=False,          # bool: 是否缓存响应
    cache_kwargs={},                # dict: RedisCache 额外参数

    # === Redis（多实例共享状态） ===
    redis_host=None,                # str: Redis 主机
    redis_port=None,                # int: Redis 端口
    redis_password=None,            # str: Redis 密码
    redis_url=None,                 # str: Redis URL（替代 host/port/password）

    # === 上下文管理 ===
    context_length_manager=None,    # str: "ljm"(推荐) / "simple" / None
    default_max_parallel_requests=None,  # int: 默认并发上限

    # === Fallback ===
    fallbacks=[],                   # list[dict]: 跨模型 fallback
    default_fallbacks=None,         # list: 全局兜底模型
    context_window_fallbacks=[],    # list[dict]: 超长上下文 fallback
    content_policy_fallbacks=[],    # list[dict]: 内容策略 fallback

    # === 路由组 ===
    routing_groups=[],              # list[dict]: 按模型组不同策略

    # === 高级 ===
    enable_weighted_failover=False, # bool: 同模型组内失败转移
    max_fallbacks=5,                # int: max_fallbacks 上限
    set_verbose=False,              # bool: 调试日志
    debug_level="INFO",             # str: DEBUG/INFO/WARNING
    model_group_alias={},           # dict: 模型别名映射
)
```
...
# 异步流式
stream = await router.acompletion(model="gpt-4o", messages=[...], stream=True)
async for chunk in stream:
    print(chunk)
# Embedding
response = await router.aembedding(model="text-embedding-3-small", input=["Hello"])
# Image Generation
response = await router.aimage_generation(model="dall-e-3", prompt="...")
# Text Completion（旧版 API）
response = await router.atext_completion(model="gpt-3.5-turbo-instruct", prompt="...")
```

### 22.3 Proxy 模式核心配置参数

Proxy 模式通过 `config.yaml` 配置，最终映射为 Router 的参数。

#### 22.3.1 顶层配置块

| 配置块 | 说明 | 映射到 |
|-------|------|--------|
| `model_list` | 模型部署定义 | Router 的 `model_list` |
| `router_settings` | 路由/重试/熔断/策略 | Router 构造函数参数 |
| `litellm_settings` | 模块级全局设置 | `litellm.set_*` 全局函数 |
| `general_settings` | 服务级设置（master key、DB 等） | 服务配置 |
| `environment_variables` | 环境变量声明 | `os.environ` |
| `credential_list` | 集中凭证管理 | 模型参数复用 |

#### 22.3.2 `router_settings` 完整参数
...
#### 22.3.3 `litellm_settings` 完整参数

```yaml
litellm_settings:
  num_retries: 3                        # 模块级重试次数
  request_timeout: 10                   # 请求超时
  drop_params: true                     # 删掉不支持的多余参数
  set_verbose: false                    # 调试日志
  api_base: ""                          # 全局默认 api_base
  cache: true                           # 启用缓存
  cache_params: {"type": "redis", "ttl": 3600}  # 缓存参数
  default_fallbacks: ["gpt-4o-mini"]    # 全局兜底
  fallbacks: [{"gpt-4o": ["claude-sonnet", "gpt-4o-mini"]}]
  context_window_fallbacks: [{"gpt-4o": ["gpt-4o-32k"]}]
  content_policy_fallbacks: [{"azure-gpt-4o": ["anthropic-claude-sonnet"]}]
  success_callback: ["langfuse"]        # 成功回调
  failure_callback: ["sentry"]           # 失败回调
  service_callback: ["slack"]            # 告警回调
  callbacks: []                          # 自定义回调
  turn_off_message_logging: false        # 关闭消息日志（隐私场景）
  redact_messages_in_exceptions: false   # 异常中脱敏消息
  disable_end_user_cost_tracking: false  # 关闭端用户成本追踪
```

#### 22.3.4 `general_settings` 完整参数

```yaml
general_settings:
  master_key: "sk-your-master-key"       # 主 Key，所有请求需携带
  database_url: ""                       # PostgreSQL 连接串
  database_connection_pool_limit: 10     # 连接池上限
  database_connection_timeout: 60        # 连接超时
  database_socket_timeout: 300           # Socket 超时
  database_connect_timeout: 15           # 建连超时
  alerting: ["slack"]                    # 告警通道
  alerting_threshold: 300                # 慢请求告警阈值（秒）
  otel: true                             # 启用 OpenTelemetry
  custom_auth: "my_auth_function"        # 自定义认证函数
  max_request_size_mb: 10                # 请求体大小上限
  proxy_batch_write_interval: 10         # 批量写入间隔
  proxy_log_to_batch_write: true         # 启用批量日志写入
  disable_spend_logs: false              # 关闭花费日志
  disable_add_to_db_on_proxy_start: false  # 启动时不写入DB
  pass_through_headers: ["X-MaaS-Trace"] # 透传客户端请求头到上游
```

### 22.4 SDK vs Proxy 选型决策

| 场景 | 推荐模式 | 原因 |
|------|---------|------|
| 微服务内部调用，已自建路由/认证 | SDK | 零额外组件，进程内调用最低延迟 |
| 需要统一 Key 治理、预算、spend tracking | Proxy | 内置虚拟 Key 和成本追踪 |
| 多语言团队（非 Python） | Proxy | 只需 OpenAI SDK，语言无关 |
| 已有独立 API Gateway，只需协议翻译 | SDK（`acompletion` 直接调用） | 轻量集成 |
| 需要 UI 管理模型和 Key | Proxy | 内置 Admin UI |
| 高并发（数千 QPS） | Proxy + Redis | 多实例共享冷却/限流状态 |
| 需要自定义路由逻辑 | SDK（`Router` + 自定义策略） | 更灵活，可嵌入自定义代码 |
| 快速原型/POC | Proxy | 配置即用，零代码 |

### 22.5 关键结论

Proxy 模式是 SDK 模式的**超集**——它在 Router 外面套了一层 HTTP API + 认证 + 治理 + UI。当你不需要 HTTP 独立服务和治理能力时，SDK 模式更轻量；当你需要企业级多租户治理时，Proxy 模式是正确选择。**MaaS 的 adapter-service 采用的就是 SDK 模式**，在进程内直接实例化 `Router`，同时自建 KeyPool 和 RetryEngine 替代 Proxy 内置的虚拟 Key 和重试机制。

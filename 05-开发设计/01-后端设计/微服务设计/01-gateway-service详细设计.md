...
**变更说明：** V2.0 新增 SSO JWT 支持、合规前置过滤钩子、零数据保留模式信号传播、完整 Trace 上下文注入、客户 KMS 头转发；独立出 compliance-service 和 auth-service。
...
| 职责域 | 具体能力 |
|--------|---------|
| **接入层** | HTTP/HTTPS 统一接入，OpenAI 兼容端点（`/v1/*`），流式 SSE 代理 |
...
| **请求标准化** | 校验 JSON Schema，补全缺省字段，注入 zero_retention / kms_key_id 标记 |
| **路由分发** | gRPC → routing-service，代理同步 / SSE 流式响应 |
